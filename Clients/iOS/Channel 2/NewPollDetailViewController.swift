        ...
        if(selectedIndex != -1) {
            ...
            Async.background {
                if(!self.voter.vote(self.selectedIndex + 1)) {
                    ...
                    Async.main {
                        NSUserDefaults.standardUserDefaults().setBool(true, forKey: "shouldRecheckPolls"); // Makes the Poll table refresh when we return to it
                        // Update the button
                        ...
                        self.voting = false;
                        // Show a popup
                        Popup().banner("Vote Recorded!", message: "Thanks for voting! Tap to share!", onClick: {
                            // Share poll/promote the app with iOS share sheet
                            let shareContent = "I voted on " + self.poll.prompt! + " in the Channel 2 App!" + " Download it for free @ https://appstore.com/bshstv"
