...
class Popup: NSObject {
    ...
    internal func banner(title:String, message:String, onClick: (() -> Void)?) {
        let notificationManager = LNRNotificationManager();
        guard notificationManager.activeNotification == nil else {
            notificationManager.dismissActiveNotification({ (_) in
                self.banner(title, message: message, onClick: onClick)
            })
            
            return;
        }
        notificationManager.showNotification(title, body: message) { (_) in
